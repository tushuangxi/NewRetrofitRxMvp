package com.xdw.retrofitrxmvpdemo.libding.weigit.view;
...
//            imgView.setImageUrl(lastUrl, mImageLoader);
            Glide.with(PadApplication.getContext()).load(lastUrl).diskCacheStrategy(DiskCacheStrategy.ALL).centerCrop().placeholder(R.mipmap.homebg).into(imgView);
...
//            imgView.setImageUrl(notice.getPicUrl(), mImageLoader);
            Glide.with(PadApplication.getContext()).load(notice).diskCacheStrategy(DiskCacheStrategy.ALL).centerCrop().placeholder(R.mipmap.homebg).into(imgView);
            this.container.addView(linearLayout);
            position ++;
        }
        if (size > 1) {
...
//            imgView.setImageUrl(firstUrl, mImageLoader);
            Glide.with(PadApplication.getContext()).load(firstUrl).diskCacheStrategy(DiskCacheStrategy.ALL).centerCrop().placeholder(R.mipmap.homebg).into(imgView);
